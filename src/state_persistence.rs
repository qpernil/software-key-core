//! Unix state-file locking, atomic replacement, and durable mutation scheduling.
...
/// Durability policy for recorded mutations.
///
/// Immediate mode waits for the corresponding write before a mutation receipt
/// completes; batched mode schedules a write after the first pending mutation.
...
/// Owns a state value and its background persistence thread.
///
/// The caller owns the state-file lock, encoding format, and restoration policy.
/// Mutations must be reported through a handle after updating the state.
pub struct StatePersistence<T> {
    handle: StatePersistenceHandle<T>,
    thread: Option<thread::JoinHandle<io::Result<()>>>,
}

/// Cloneable access to the state and its durable-mutation schedule.
...
/// A recorded mutation's completion or persistence-failure result.
...
/// Replace one mutable state image and sync both the file and its directory.
///
/// The caller must keep the containing directory private and hold the
/// appropriate [`StateLock`] for the lifetime of state restoration and writes.
...
mod tests {
    ...
    #[test]
    fn state_lock_does_not_follow_symlinks() {
        let directory = std::env::temp_dir().join(format!(
            "software-key-core-state-lock-symlink-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&directory);
        fs::create_dir(&directory).unwrap();
        ...
        let link = directory.join("device.lock");
        symlink(&target, &link).unwrap();

        assert!(StateLock::acquire(link).is_err());
